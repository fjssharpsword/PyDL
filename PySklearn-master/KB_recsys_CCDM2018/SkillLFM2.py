        ...
        for step in xrange(steps):
            for i in xrange(len(R)):
                for j in xrange(len(R[i])):
                    if R[i][j] > 0:
                        eij = R[i][j] - np.dot(P[i,:]+SU[i]*SI[j],Q[:,j])
                        for k in xrange(K):
                            P[i][k] = P[i][k] + alpha * (2 * eij * Q[k][j] - beta * P[i][k])
                            Q[k][j] = Q[k][j] + alpha * (2 * eij * (P[i][k]+SU[i]*SI[j]) - beta * Q[k][j])
                            SI[j]   = SI[j]   + alpha * (2 * eij * SU[i]* Q[k][j]   - beta * SI[j])
            eR = np.dot(P,Q)
            e = 0
            for i in xrange(len(R)):
                for j in xrange(len(R[i])):
                    if R[i][j] > 0:
                        e = e + pow(R[i][j] - np.dot(P[i,:]+SU[i]*SI[j],Q[:,j]), 2)
                        for k in xrange(K):
                            e = e + (beta/2) * ( pow(P[i][k]+SU[i]*SI[j],2) + pow(Q[k][j],2) ) 
            if e < 0.001:
                break
        return P, Q.T, SI
    ...
    for K in [10,20,50]:#隐因子
        for steps in [2,100,200]:#迭代次数
            ...
            for index, row in test.iterrows(): # 获取每行的值
                ...
                i = int(row['csr'])
                j = int(row['ke'])
                pRating=np.dot(nP[i,:]+SU[i]*nSI[j],nQ[:,j])
                TRating=row['rating']
