...
class BiasSVD:
    ...
    """
    def matrix_factorization(self,R, P, Q, SU, SI, K, beta1, beta2, steps=100, alpha=0.0002):
        Q = Q.T
        for step in xrange(steps):
            for i in xrange(len(R)):
                for j in xrange(len(R[i])):
                    if R[i][j] > 0:
                        eij = R[i][j] - np.dot(P[i,:],Q[:,j])-SU[i]-SI[j,0]
                        for k in xrange(K):
                            P[i][k] = P[i][k] + alpha * (2 * eij * Q[k][j] - beta1 * P[i][k])
                            Q[k][j] = Q[k][j] + alpha * (2 * eij * P[i][k] - beta1 * Q[k][j])
                            SI[j,0] = SI[j,0] + alpha * (2 * eij    - beta2 * SI[j,0])
            eR = np.dot(P,Q)
            e = 0
            for i in xrange(len(R)):
                for j in xrange(len(R[i])):
                    if R[i][j] > 0:
                        e = e + pow(R[i][j] - np.dot(P[i,:],Q[:,j])-SU[i]-SI[j,0], 2)
                        for k in xrange(K):
                            e = e + (beta1/2) * ( pow(P[i][k],2) + pow(Q[k][j],2) )+ (beta2/2) *( pow(SU[i]+SI[j,0],2) )
            if e < 0.001:
                break
        return P, Q.T, SI
...
if __name__ == "__main__":    
    ...
    sr_mean=df_kb.groupby(['csr'])['num'].mean()
    SU = (sr_count+1)*1.0/(sr_mean+1)#每个用户的技能水平
    SU=SU.apply(lambda x: (x-SU.min()+1)*1.0/(SU.max()-SU.min()+1))
    #阅读次数做区间缩放(0,1]
    ...
    #SVD分解，随机梯度下降求解P、Q  
    bsvd=BiasSVD(method='SVDPlus')#传递方法
    N = len(R)
    M = len(R[0])
    print ("%3s%20s%20s" % ('beta1','beta2','RMSE'))
    K=5
    for beta2 in [0.001,0.005,0.01,0.05,0.1,0.5,1,5,10]:#隐因子
        for beta1 in [0.002]:#迭代次数
            P = np.random.rand(N,K)
            Q = np.random.rand(M,K)
            SI = np.random.rand(M,1) #每个知识点的偏置
            #SVD分解
            nP, nQ, nSI = bsvd.matrix_factorization(R, P, Q, SU, SI, K, beta1, beta2)
            nR = np.dot(nP, nQ.T)  
            #RMSE评估   
            squaredError = []
            for index, row in test.iterrows(): # 获取每行的值
                pRating=nR[int(row['csr'])][int(row['ke'])]+SU[int(row['csr'])]+nSI[int(row['ke']),0] #获取预测值
                TRating=row['rating']
                error=TRating-pRating
                squaredError.append(error * error)
            RMSE =sqrt(sum(squaredError) / len(squaredError))#均方根误差RMSE 
            print ("%3.6f%20.6f%20.6f" % (beta1,beta2,RMSE))
