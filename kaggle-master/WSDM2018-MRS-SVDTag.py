...
if __name__ == "__main__":   
    ...
    #6：标签SVD后生成的训练集和测试集
    #6.1：训练集
    train['source_system_tab']=train.apply(lambda x:algo_source_system_tab.predict(x['msno'],x['source_system_tab']).est,axis=1)
    train['source_screen_name']=train.apply(lambda x:algo_source_screen_name.predict(x['msno'],x['source_screen_name']).est,axis=1)
    train['source_type']=train.apply(lambda x:algo_source_type.predict(x['msno'],x['source_type']).est,axis=1)
    train['song_area']=train.apply(lambda x:algo_area.predict(x['msno'],x['song_area']).est,axis=1)
    train['song_year']=train.apply(lambda x:algo_year.predict(x['msno'],x['song_year']).est,axis=1)
    train['language']=train.apply(lambda x:algo_lang.predict(x['msno'],x['language']).est,axis=1)
    ...
    train['song_length']=train.apply(lambda x:songlengthDiscretization(x),axis=1)
    def genreidsGetMax(x):
        max_genre=0
        genre_tags=str(x['genre_ids']).split('|')
        for genre_tag in genre_tags:
            est_genre = algo_genre.predict(x['msno'],genre_tag).est
            if (est_genre>max_genre):
                max_genre=est_genre
        return max_genre
    train['genre_ids']=train.apply(lambda x:genreidsGetMax(x),axis=1)
    def artistnameGetMax(x):
        max_art=0
        for art_tag in re.split('/|\\\|;|\||&',str(x['artist_name'])):
            est_art = algo_artist.predict(x['msno'],art_tag).est
            if (max_art<est_art):
                max_art=est_art
        return max_art
    train['artist_name']=train.apply(lambda x:artistnameGetMax(x),axis=1)
    def composerGetMax(x):
        max_com=0
        for com_tag in re.split('/|\\\|;|\||&',str(x['composer'])):
            est_com = algo_composer.predict(x['msno'],com_tag).est
            if (max_com<est_com):
                max_com=est_com
        return max_com
    train['composer']=train.apply(lambda x:composerGetMax(x),axis=1)
    def lyricistGetMax(x):
        max_lyr=0
        for lyr_tag in re.split('/|\\\|;|\||&',str(x['lyricist'])):
            est_lyr = algo_lyricist.predict(x['msno'],lyr_tag).est
            if (max_lyr<est_lyr):
                max_lyr=est_lyr
        return max_lyr
    train['lyricist']=train.apply(lambda x:lyricistGetMax(x),axis=1)    
    train['city']=train.apply(lambda x:algo_city.predict(x['city'],x['song_id']).est,axis=1)    
    train['bd']=train.apply(lambda x:algo_bd.predict(x['bd'],x['song_id']).est,axis=1)     
    train['gender']=train.apply(lambda x:algo_gender.predict(x['gender'],x['song_id']).est,axis=1)
    train['registered_via']=train.apply(lambda x:algo_registered_via.predict(x['registered_via'],x['song_id']).est,axis=1)
    train['membership_days']=train.apply(lambda x:algo_membership_days.predict(x['membership_days'],x['song_id']).est,axis=1)
    train=train[['source_system_tab','source_screen_name','source_type','song_area','song_year','language','song_length','genre_ids','artist_name','composer','lyricist','city','bd','gender','registered_via','membership_days','target']]
    #6.2：测试集   
    test['source_system_tab']=test.apply(lambda x:algo_source_system_tab.predict(x['msno'],x['source_system_tab']).est,axis=1)
    test['source_screen_name']=test.apply(lambda x:algo_source_screen_name.predict(x['msno'],x['source_screen_name']).est,axis=1)
    test['source_type']=test.apply(lambda x:algo_source_type.predict(x['msno'],x['source_type']).est,axis=1)
    test['song_area']=test.apply(lambda x:algo_area.predict(x['msno'],x['song_area']).est,axis=1)
    test['song_year']=test.apply(lambda x:algo_year.predict(x['msno'],x['song_year']).est,axis=1)
    test['language']=test.apply(lambda x:algo_lang.predict(x['msno'],x['language']).est,axis=1)
    def songlengthDiscretization(x):
        if x['song_length']<=_mean_song_length:
            return algo_len.predict(x['msno'],1).est
        else:
            return algo_len.predict(x['msno'],2).est
    test['song_length']=test.apply(lambda x:songlengthDiscretization(x),axis=1)
    def genreidsGetMax(x):
        max_genre=0
        genre_tags=str(x['genre_ids']).split('|')
        for genre_tag in genre_tags:
            ...
            if (est_genre>max_genre):
                max_genre=est_genre
        return max_genre
    test['genre_ids']=test.apply(lambda x:genreidsGetMax(x),axis=1)
    def artistnameGetMax(x):
        max_art=0
        for art_tag in re.split('/|\\\|;|\||&',str(x['artist_name'])):
            est_art = algo_artist.predict(x['msno'],art_tag).est
            if (max_art<est_art):
                max_art=est_art
        return max_art
    test['artist_name']=test.apply(lambda x:artistnameGetMax(x),axis=1)
    def composerGetMax(x):
        max_com=0
        for com_tag in re.split('/|\\\|;|\||&',str(x['composer'])):
            est_com = algo_composer.predict(x['msno'],com_tag).est
            if (max_com<est_com):
                max_com=est_com
        return max_com
    test['composer']=test.apply(lambda x:composerGetMax(x),axis=1)
    def lyricistGetMax(x):
        max_lyr=0
        for lyr_tag in re.split('/|\\\|;|\||&',str(x['lyricist'])):
            est_lyr = algo_lyricist.predict(x['msno'],lyr_tag).est
            if (max_lyr<est_lyr):
                max_lyr=est_lyr
        return max_lyr
    test['lyricist']=test.apply(lambda x:lyricistGetMax(x),axis=1)    
    test['city']=test.apply(lambda x:algo_city.predict(x['city'],x['song_id']).est,axis=1)    
    test['bd']=test.apply(lambda x:algo_bd.predict(x['bd'],x['song_id']).est,axis=1)     
    test['gender']=test.apply(lambda x:algo_gender.predict(x['gender'],x['song_id']).est,axis=1)
    test['registered_via']=test.apply(lambda x:algo_registered_via.predict(x['registered_via'],x['song_id']).est,axis=1)
    test['membership_days']=test.apply(lambda x:algo_membership_days.predict(x['membership_days'],x['song_id']).est,axis=1)
    test=test[['source_system_tab','source_screen_name','source_type','song_area','song_year','language','song_length','genre_ids','artist_name','composer','lyricist','city','bd','gender','registered_via','membership_days','id']]
    ################################################################################################
    #7：lgbm模型训练
    #7.1：训练集划分
